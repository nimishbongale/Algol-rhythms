...
void heapify(int h[],int n)
{
	...
	for(i=n/2;i>=1;i--)//i = looping variable
	{
	  ...
	  while(!(heap)&&(2*k)<=n1)//while it is not a heap, and left child position is lesser than the maximum position of farthest child
	  {
...
int main()
{
 ...
 for(i=1;i<=n;i++)
  ...
  for(i=1;i<=n;i++)
  h[i]=rand();//h[] stores the elements*/
